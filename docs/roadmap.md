...
| M2 | 评测材料准备 | 准备分层题集、标准答案、错误标注和最小自动校验工具 | 8题/25候选、工具及隔离验证完成；3条gold已人工确认，余22条待审、未正式冻结 |
| M3 | 效果验证与分析 | 通过真实 Hy3 实验、独立答案检查和人工抽检验证有效性 | 旧开发3次HTTP200但0次严格解析通过；已停止扩大，新独立评测v2先离线验证 |
...
最新执行：用户已确认账户条件；旧开发s001/s002/s003各调用一次，总计36420 token，未知0，剩余263580 token/35次。三条均schema_invalid、无可用完整代码，停止扩大，不进入正式冻结/保留集。作者实际复核这三条gold成立；解码后8处snippet全部匹配，旧转义错误描述已追加撤回。独立[greedy-evaluation-v2](evaluation-v2.md)保留严格语义、提供完整可校验示例和错误路径，仅先离线验证；本轮新增真实调用0。若后续决定验证新契约，最多3条开发调用，必须复用原账本，不能用新目录重置额度。详见[M3开发记录](journal/m3-development-pilot.md)和[三条人工证据](../evaluation/reviews/s001-s003-20260828.json)，不能视为全部25条或模型完整解法已审核。
