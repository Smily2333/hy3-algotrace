...
**当前状态：M1完成；M2技术准备已交付，M3账户门槛待核实，M4部分交付（2026-08-27）。** 网页 v2 只要求题面和代码，不执行用户代码、不连接OJ、不读取gold。离线评测独立使用 evaluation-v1 与批准的无网络隔离环境。旧pilot不作为新模式效果证据，通用CandidateRunner继续暂缓。实际测试、SHA与CI见 [最终验证记录](docs/journal/m4-delivery.md)。
...
| M3 | 真实 Hy3 实验、指标、人工抽检和失败分析 | 0次调用；账户额度条件与人工审查待落实 |
